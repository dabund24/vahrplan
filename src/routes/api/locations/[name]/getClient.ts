...
export class GetLocationsApiClient extends PathParamSettable<string, ParsedLocation[]>()(
	ApiClient<string, ParsedLocation[], "GET", RequestEvent>
) {
	protected override readonly methodType = "GET";
	protected override readonly route = "/api/locations/[name]";
	...
	protected override formatUrlPath(content: string): `/api/${string}` {
		return `/api/locations/${encodeURIComponent(content)}`;
	}
