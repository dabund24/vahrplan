...
async function generateDiagramShortUrl(formData: DisplayedFormData): Promise<URL | undefined> {
	const diagramRequestData = apiClient("GET", "/de/dbnav/api/diagram").formDataToRequestData(
		formData
	);
	...
	const response = await apiClient("PUT", "/de/dbnav/api/diagram/shorturl").request(
	...
	return apiClient("GET", "/de/dbnav/api/diagram/shorturl/[shortDiagramId]").formatNonApiUrl(
		response.content
	);
}
